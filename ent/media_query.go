...
type MediaQuery struct {
	...
	withOwnerProfilePicture *UserProfileQuery
	withOwnerBanner         *UserProfileQuery
...
func (mq *MediaQuery) QueryThreads() *ThreadQuery {
...
// QueryOwnerProfilePicture chains the current query on the "owner_profile_picture" edge.
func (mq *MediaQuery) QueryOwnerProfilePicture() *UserProfileQuery {
	query := (&UserProfileClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryOwnerBanner chains the current query on the "owner_banner" edge.
func (mq *MediaQuery) QueryOwnerBanner() *UserProfileQuery {
	query := (&UserProfileClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (mq *MediaQuery) Clone() *MediaQuery {
	...
	return &MediaQuery{
		...
		withThreads:             mq.withThreads.Clone(),
		withOwnerProfilePicture: mq.withOwnerProfilePicture.Clone(),
		withOwnerBanner:         mq.withOwnerBanner.Clone(),
...
func (mq *MediaQuery) WithThreads(opts ...func(*ThreadQuery)) *MediaQuery {
	query := (&ThreadClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withThreads = query
	return mq
}

// WithOwnerProfilePicture tells the query-builder to eager-load the nodes that are connected to
// the "owner_profile_picture" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MediaQuery) WithOwnerProfilePicture(opts ...func(*UserProfileQuery)) *MediaQuery {
	query := (&UserProfileClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withOwnerProfilePicture = query
	return mq
}

// WithOwnerBanner tells the query-builder to eager-load the nodes that are connected to
// the "owner_banner" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MediaQuery) WithOwnerBanner(opts ...func(*UserProfileQuery)) *MediaQuery {
	query := (&UserProfileClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withOwnerBanner = query
	return mq
}
...
func (mq *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	var (
		...
		loadedTypes = [3]bool{
			mq.withThreads != nil,
			mq.withOwnerProfilePicture != nil,
			mq.withOwnerBanner != nil,
		}
	)
	...
	if query := mq.withThreads; query != nil {
		if err := mq.loadThreads(ctx, query, nodes,
			func(n *Media) { n.Edges.Threads = []*Thread{} },
			func(n *Media, e *Thread) { n.Edges.Threads = append(n.Edges.Threads, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withOwnerProfilePicture; query != nil {
		if err := mq.loadOwnerProfilePicture(ctx, query, nodes,
			func(n *Media) { n.Edges.OwnerProfilePicture = []*UserProfile{} },
			func(n *Media, e *UserProfile) { n.Edges.OwnerProfilePicture = append(n.Edges.OwnerProfilePicture, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withOwnerBanner; query != nil {
		if err := mq.loadOwnerBanner(ctx, query, nodes,
			func(n *Media) { n.Edges.OwnerBanner = []*UserProfile{} },
			func(n *Media, e *UserProfile) { n.Edges.OwnerBanner = append(n.Edges.OwnerBanner, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MediaQuery) loadThreads(ctx context.Context, query *ThreadQuery, nodes []*Media, init func(*Media), assign func(*Media, *Thread)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(media.ThreadsTable)
		s.Join(joinT).On(s.C(thread.FieldID), joinT.C(media.ThreadsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(media.ThreadsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(media.ThreadsPrimaryKey[1]))
...
func (mq *MediaQuery) loadOwnerProfilePicture(ctx context.Context, query *UserProfileQuery, nodes []*Media, init func(*Media), assign func(*Media, *UserProfile)) error {
...
func (mq *MediaQuery) loadOwnerBanner(ctx context.Context, query *UserProfileQuery, nodes []*Media, init func(*Media), assign func(*Media, *UserProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Media)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
